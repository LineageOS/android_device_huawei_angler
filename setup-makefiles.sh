...
VENDORLIBS=`echo "$BLOBLIST" | grep '\-vendor\/lib' | cut -d'-' -f2 | head -1`
...
LINEEND=" \\"
COUNT=`echo "$BLOBLIST" | grep '\-vendor\/lib' | wc -l`
for VENDORLIB in `echo "$BLOBLLIST" | grep '\-vendor\/lib' | cut -d'/' -f3`;do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
